...
import sys, time
import torch as th
from dgl.distgnn.dist_sampling import dist_sampling as DistNeighborSampler
...
from dgl.distgnn.utils import chunkit, cumsum
...
#####################################################################################
## Modified distgnn_mb wrapper using distributed sampling
#####################################################################################
class distgnn_mb:
    ...
    def init_setup(self, fan_out, model, optimizer):
        ...
        if self.mode == 'iels':
            ...
            if len(self.ntypes_id) > 1:
                self.dist_sampler = DistNeighborSampler(self.pb, self.rank, self.num_parts, \
                                                        fanouts, True)
            else:
    ...
    def adjust_batch(self, mask):
        ...
        #gtrain_nodes = th.nonzero(self.pb.gtrain_mask, as_tuple=True)[0]
        #acc, count = chunkit(gtrain_nodes, self.num_parts)
        #tsize = count[self.rank]
        #self.tr_nid_part = gtrain_nodes[acc[self.rank] : acc[self.rank+1]]
        self.tr_nid_part = self.get_train
        tsize = self.tr_nid_part.shape[0]
        #assert tsize == self.tr_nid_part.shape[0]
        #acc, count = chunkit(gtrain_nodes, self.num_parts)
        
        my_steps = ceil(tsize/self.batch_size)
        #my_steps = th.tensor(t)
        ...
        self.bs_all = cumsum(self.bs_all)
    ...
    def ielspp_epoch_setup(self):
        self.gobj.reset()
        self.tr_nid_perm, idx = shuffle(self.tr_nid_part)
        self.labels_perm = self.labels[idx]
    ...
    def ielspp_batch_setup_startup(self):  # Added 'i' as an argument
        for i in range(self.delay):
            ...
            seeds_o = self.tr_nid_perm[st : ed ]
            labels = self.labels_perm[st:ed]  
            #seeds_p = self.tr_nid_p[st : ed] 

            tic = time.time()
            if len(self.ntypes_id) > 1:
                seeds_od = {self.category: seeds_o}
            else:
                seeds_od = seeds_o

            #_, output_nodes, blocks = self.sampler.sample_blocks(self.g_orig, seeds_od)
            blocks = self.dist_sampler.sample_neighbors(self.g, seeds_od)
            
            self.ticd += time.time() - tic
            input_nodes = blocks[0].srcdata[dgl.NID]
            batch = ielspp_batch(self.gobj, input_nodes)
            batch.blocks = blocks
            batch.labels = labels_o
            batch.seeds = seeds_o             ##check this
    ...
    def ielspp_batch_setup(self, i):  
        if i < self.train_step_count:
            ...
            batch = ielspp_batch(self.gobj, input_nodes)
            batch.blocks = blocks
            batch.labels = labels  # 'labels' needs to be defined
            batch.seeds = seeds_o #seeds_o
            batch.input_nodes = input_nodes
    ...
    def get_labels(self):
        ...
        gtrain_nodes = th.nonzero(self.pb.gtrain_mask, as_tuple=True)[0]
        labels = self.pb.glabels[gtrain_nodes]
        acc, count = chunkit(gtrain_nodes, self.num_parts)
        return labels[acc[self.rank] : acc[self.rank+1]]            

    
    @property
    def get_feats(self):
        features  = self.node_feats['feats']
        return features
    ...
    def get_train(self):
        ...
        gtrain_nodes = th.nonzero(self.pb.gtrain_mask, as_tuple=True)[0]
        acc, count = chunkit(gtrain_nodes, self.num_parts)
        return gtrain_nodes[acc[self.rank] : acc[self.rank+1]]
...
class distgnn_mb_inf:
    #def __init__(self, pb, args, category=''):
    def __init__(self, pb, gobj, args, category=''):
        self.mode = args.mode        
        if args.mode != 'iels':
            print('wrong mode provided....exiting')
            sys.exit(0)
    ...
    def adjust_batch(self, mask):
        ...
        #tsize = count[self.rank]
        #self.tr_nid_part = gtrain_nodes[acc[self.rank] : acc[self.rank+1]]
        #self.ts_nid_part = self.get_test
        #val_nids = int(self.ts_nid_part.shape[0] * self.val_fraction)
        #self.ts_nid_part = self.ts_nid_part[:val_nids]
        self.ts_nid_part = self.get_test
        val_nids = self.ts_nid_part.shape[0]
        
        #acc, count = chunkit(self.ts_nid_part, self.num_parts)
        #val_nids = count[self.rank]
        #self.tr_nid_part = self.ts_nid_part[acc[self.rank] : acc[self.rank+1]]
        batch_size = self.val_batch_size
                
        #tsize = self.ts_nid_part.shape[0]
        #assert tsize == self.tr_nid_part.shape[0]
        #acc, count = chunkit(gtrain_nodes, self.num_parts)
        
        my_steps = ceil(val_nids/self.batch_size)
        #my_steps = th.tensor(t)
        my_steps_t = th.tensor(my_steps)
        dist.barrier()
        dist.all_reduce(my_steps_t, op=dist.ReduceOp.MAX)
    ...
    def ielspp_epoch_setup(self):
        self.gobj.reset()
        self.ts_nid_perm = self.ts_nid_part
        self.labels_perm = self.labels
        ##self.tr_nid_o = self.po_map[self.tr_nid_p]
    ...
    def ielspp_batch_setup(self, i):  
        if i < self.val_step_count:
            st, ed = self.bs_all[i], self.bs_all[i+1]
            seeds_o = self.ts_nid_perm[st : ed ]
            labels = self.labels_perm[st : ed]  
            ##seeds_p = self.tr_nid_p[st : ed]  
    ...
    def get_labels(self):
        ...
        #    labels = self.node_feats['labels'].long()
        if self.rank == 0:
            print("test size: ", self.pb.gtest_mask.size())
            print("labels size: ", self.pb.glabels.size())
        gtest_nodes = th.nonzero(self.pb.gtest_mask, as_tuple=True)[0]
        labels = self.pb.glabels[gtest_nodes]
        
        ts_nids = int(gtest_nodes.shape[0] * self.val_fraction)
        gtest_nodes = gtest_nodes[:ts_nids]
        acc, count = chunkit(gtest_nodes, self.num_parts)
        return labels[acc[self.rank] : acc[self.rank+1]]            
    ...
    def get_test(self):
        ...
        gtest_nodes = gtest_nodes[:ts_nids]
        acc, count = chunkit(gtest_nodes, self.num_parts)        
        return gtest_nodes[acc[self.rank] : acc[self.rank+1]]
