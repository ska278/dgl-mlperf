...
if __name__ == "__main__":
    ...
    for p in range(args.num_parts):
        pname = osp.join(args.part_path, 'part') + str(p)
    ...
    out_path = osp.join(args.part_path, 'IGBH.json')
    _dump_part_config(out_path, pm)

    for p in range(args.num_parts):
        gpath = osp.join(args.part_path, 'part') + str(p)
        gpath = osp.join(gpath, 'graph.pt')
        g = torch.load(gpath)
        ogpath = osp.join(args.part_path, 'part') + str(p)
