...
######################################################################################
## DistributedSampling: New code using partition graph strutcure and distributed sampling for MB
## ---------------------------------------------
...
class iels_master():
    ...
    def iels_init(self, pb, args):        
        ...
        self.all_ntypes = pb.all_ntypes
        self.all_etypes = pb.all_etypes
        self.N = 0
        #for nt in self.all_ntypes:
        #    self.N += pb.num_nodes_orig[nt]
        
        self.ntypes_id, self.acc_onodes, offset = {}, {}, 0
        for keys, nt in enumerate(pb.all_ntypes):
            self.ntypes_id[nt] = keys
            self.acc_onodes[nt] = offset
            print('nt: ', nt, 'offset: ', offset, self.acc_onodes)
            offset += int(pb.num_nodes_orig[nt])

        #print('acc_onodes: ', self.acc_onodes)
        #print('num_nodes_orig: ', pb.num_nodes_orig)
        self.N = offset

        self.pn = self.ndata['feats'].shape[0]
        self.feats = self.ndata['feats']
        self.dtype = self.feats.dtype
        self.feat_size = self.feats.shape[1]
        self.part_sn_onid = self.ndata['orig']
        self.part_sn_gnid = th.tensor(list(th.arange(self.pn)), dtype=th.int32)

        if self.rank == 0:
            print('partition nodes: ', self.pn)
            print('orig nodes: ', self.N)
...
class ielspp_batch():
    def __init__(self, part, input_nodes):
        ...
        self.batch_feats = {}
        for nt in self.part.all_ntypes:
    ...
    def find_remote_bnodes(self, ntype):
        if isinstance(self.input_nodes, dict):
            ...
            #print('acc_onodes: ', self.acc_onodes)
            #print('ntype: ', ntype, 'offset: ', offset)
            rn_batch = th.nonzero(self.part.local_sn_db[ \
...
#######################################################################################
## LocalSampling: original code using original graph structure for smapling
'''
class iels_master():
    def __init__(self):
        pass
    ...
    def iels_init(self, pb, args):
        ...
        self.all_ntypes = pb.g_orig.ntypes
        self.ntypes_id, self.acc_onodes, offset = {}, {}, 0
